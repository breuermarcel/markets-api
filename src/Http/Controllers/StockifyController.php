...
namespace Stockify\Http\Controllers;

use Carbon\Carbon;
use Stockify\Models\Stock;
use Illuminate\Support\Str;
use Illuminate\Support\Facades\Validator;

class StockifyController extends Controller
{
    /**
    ...
    public function load()
    {
        ...
        if (request()->has("symbol")) {
            $stock = Stock::find(Str::upper(request()->get("symbol")));
                ...
                switch (request()->get("module")) {
                    case "chart":
                        ...
                        return response()->json([
                            "errors" => null,
                            "stock" => $stock,
                            "chart" => $this->getChart(request()->get("symbol"), $period)
                        ]);
                    ...
                    case "upgrade_downgrade":
                        return response()->json([
                            "errors" => null,
                            "stock" => $stock,
                            "upgrade_downgrade" => $this->getUpgradeDowngrade(request()->get("symbol"))
                        ]);
                }
    ...
    protected function getChart(string $symbol, int $period): array
    {
        ...
        if (($data["chart"]["error"] === null) && array_key_exists("adjclose", $data["chart"]["result"][0]["indicators"]["adjclose"][0])) {
            return $this->validateChart($data["chart"]["result"][0]);
        }
    ...
    public function getAssetProfile(string $symbol): array
    {
        ...
        if (($data["quoteSummary"]["error"] === null) && array_key_exists("assetProfile", $data["quoteSummary"]["result"][0])) {
    ...
    private function validateAssetProfile(array $data): array
    {
        ...
        if (array_key_exists("companyOfficers", $data)) {
            foreach ($data["companyOfficers"] as $index => $officer) {
                ...
                if (array_key_exists("totalPay", $officer) && array_key_exists("raw", $officer["totalPay"])) {
    ...
    public function getEsgScore(string $symbol): array
    {
        ...
        if (($data["quoteSummary"]["error"] === null) && array_key_exists("esgScores", $data["quoteSummary"]["result"][0])) {
            return $this->validateEsgScore($data["quoteSummary"]["result"][0]["esgScores"]);
        }
    ...
    private function validateEsgScore(array $data): array
    {
        ...
        if (array_key_exists("totalEsg", $data) && array_key_exists("raw", $data["totalEsg"])) {
            $esg["total"] = $data["totalEsg"]["raw"];
        }

        if (array_key_exists("environmentScore", $data) && array_key_exists("raw", $data["environmentScore"])) {
            $esg["environment"] = $data["environmentScore"]["raw"];
        }

        if (array_key_exists("socialScore", $data) && array_key_exists("raw", $data["socialScore"])) {
            $esg["social"] = $data["socialScore"]["raw"];
        }

        if (array_key_exists("governanceScore", $data) && array_key_exists("raw", $data["governanceScore"])) {
        ...
        if (array_key_exists("peerEsgScorePerformance", $data) && array_key_exists("avg", $data["peerEsgScorePerformance"])) {
            $esg["peer_total"] = $data["peerEsgScorePerformance"]["avg"];
        }

        if (array_key_exists("peerEnvironmentPerformance", $data) && array_key_exists("avg", $data["peerEnvironmentPerformance"])) {
            $esg["peer_environment"] = $data["peerEnvironmentPerformance"]["avg"];
        }

        if (array_key_exists("peerSocialPerformance", $data) && array_key_exists("avg", $data["peerSocialPerformance"])) {
            $esg["peer_social"] = $data["peerSocialPerformance"]["avg"];
        }

        if (array_key_exists("peerGovernancePerformance", $data) && array_key_exists("avg", $data["peerGovernancePerformance"])) {
    ...
    public function getIncome(string $symbol): array
    {
        ...
        if (($data["quoteSummary"]["error"] === null) && array_key_exists("incomeStatementHistory", $data["quoteSummary"]["result"][0]) && array_key_exists("incomeStatementHistory", $data["quoteSummary"]["result"][0]["incomeStatementHistory"])) {
            return $this->validateIncome($data["quoteSummary"]["result"][0]["incomeStatementHistory"]);
        }
    ...
    private function validateIncome(array $data): array
    {
        ...
        if (array_key_exists("endDate", $data) && array_key_exists("raw", $data["endDate"])) {
            $income["date"] = $data["endDate"]["raw"];
        }

        if (array_key_exists("totalRevenue", $data) && array_key_exists("raw", $data["totalRevenue"])) {
            $income["total_revenue"] = $data["totalRevenue"]["raw"];
        }

        if (array_key_exists("costOfRevenue", $data) && array_key_exists("raw", $data["costOfRevenue"])) {
            $income["cost_of_revenue"] = $data["costOfRevenue"]["raw"];
        }

        if (array_key_exists("grossProfit", $data) && array_key_exists("raw", $data["grossProfit"])) {
            $income["gross_profit"] = $data["grossProfit"]["raw"];
        }

        if (array_key_exists("researchDevelopment", $data) && array_key_exists("raw", $data["researchDevelopment"])) {
            $income["research_and_development"] = $data["researchDevelopment"]["raw"];
        }

        if (array_key_exists("sellingGeneralAdministrative", $data) && array_key_exists("raw", $data["sellingGeneralAdministrative"])) {
            $income["selling_general_administrative"] = $data["sellingGeneralAdministrative"]["raw"];
        }

        if (array_key_exists("ebit", $data) && array_key_exists("raw", $data["ebit"])) {
            $income["ebit"] = $data["ebit"]["raw"];
        }

        if (array_key_exists("interestExpense", $data) && array_key_exists("raw", $data["interestExpense"])) {
            $income["interest_expense"] = $data["interestExpense"]["raw"];
        }

        if (array_key_exists("incomeBeforeTax", $data) && array_key_exists("raw", $data["incomeBeforeTax"])) {
            $income["income_before_tax"] = $data["incomeBeforeTax"]["raw"];
        }

        if (array_key_exists("incomeTaxExpense", $data) && array_key_exists("raw", $data["incomeTaxExpense"])) {
            $income["income_tax_expense"] = $data["incomeTaxExpense"]["raw"];
        }

        if (array_key_exists("netIncome", $data) && array_key_exists("raw", $data["netIncome"])) {
    ...
    public function getCashflow(string $symbol): array
    {
        ...
        if (($data["quoteSummary"]["error"] === null) && array_key_exists("cashflowStatementHistory", $data["quoteSummary"]["result"][0]) && array_key_exists("cashflowStatements", $data["quoteSummary"]["result"][0]["cashflowStatementHistory"])) {
            return $this->validateCashflow($data["quoteSummary"]["result"][0]["cashflowStatementHistory"]["cashflowStatements"][0]);
        }
    ...
    private function validateCashflow(array $data): array
    {
        ...
        if (array_key_exists("depreciation", $data) && array_key_exists("raw", $data["depreciation"])) {
            $cashflow["depreciation"] = $data["depreciation"]["raw"];
        }

        if (array_key_exists("changeToNetincome", $data) && array_key_exists("raw", $data["changeToNetincome"])) {
            $cashflow["change_to_net_income"] = $data["changeToNetincome"]["raw"];
        }

        if (array_key_exists("changeToAccountReceivables", $data) && array_key_exists("raw", $data["changeToAccountReceivables"])) {
            $cashflow["change_to_account_receivables"] = $data["changeToAccountReceivables"]["raw"];
        }

        if (array_key_exists("changeToLiabilities", $data) && array_key_exists("raw", $data["changeToLiabilities"])) {
            $cashflow["change_to_liabilities"] = $data["changeToLiabilities"]["raw"];
        }

        if (array_key_exists("changeToInventory", $data) && array_key_exists("raw", $data["changeToInventory"])) {
            $cashflow["change_to_inventory"] = $data["changeToInventory"]["raw"];
        }

        if (array_key_exists("changeToOperatingActivities", $data) && array_key_exists("raw", $data["changeToOperatingActivities"])) {
            $cashflow["change_to_operating_activities"] = $data["changeToOperatingActivities"]["raw"];
        }

        if (array_key_exists("capitalExpenditures", $data) && array_key_exists("raw", $data["capitalExpenditures"])) {
            $cashflow["capital_expenditures"] = $data["capitalExpenditures"]["raw"];
        }

        if (array_key_exists("investments", $data) && array_key_exists("raw", $data["investments"])) {
            $cashflow["investments"] = $data["investments"]["raw"];
        }

        if (array_key_exists("dividendsPaid", $data) && array_key_exists("raw", $data["dividendsPaid"])) {
            $cashflow["dividends_paid"] = $data["dividendsPaid"]["raw"];
        }

        if (array_key_exists("netBorrowings", $data) && array_key_exists("raw", $data["netBorrowings"])) {
            $cashflow["net_borrowings"] = $data["netBorrowings"]["raw"];
        }

        if (array_key_exists("changeInCash", $data) && array_key_exists("raw", $data["changeInCash"])) {
            $cashflow["change_in_cash"] = $data["changeInCash"]["raw"];
        }

        if (array_key_exists("repurchaseOfStock", $data) && array_key_exists("raw", $data["repurchaseOfStock"])) {
            $cashflow["repurchase_of_stock"] = $data["repurchaseOfStock"]["raw"];
        }

        if (array_key_exists("totalCashFromOperatingActivities", $data) && array_key_exists("raw", $data["totalCashFromOperatingActivities"])) {
            $cashflow["total_cash_from_operating_activities"] = $data["totalCashFromOperatingActivities"]["raw"];
        }

        if (array_key_exists("totalCashflowsFromInvestingActivities", $data) && array_key_exists("raw", $data["totalCashflowsFromInvestingActivities"])) {
            $cashflow["total_cashflows_from_investing_activities"] = $data["totalCashflowsFromInvestingActivities"]["raw"];
        }

        if (array_key_exists("totalCashFromFinancingActivities", $data) && array_key_exists("raw", $data["totalCashFromFinancingActivities"])) {
    ...
    public function getBalanceSheet(string $symbol): array
    {
        ...
        if (($data["quoteSummary"]["error"] === null) && array_key_exists("balanceSheetHistory", $data["quoteSummary"]["result"][0]) && array_key_exists("balanceSheetStatements", $data["quoteSummary"]["result"][0]["balanceSheetHistory"])) {
            return $this->validateBalanceSheet($data["quoteSummary"]["result"][0]["balanceSheetHistory"]["balanceSheetStatements"][0]);
        }
    ...
    private function validateBalanceSheet(array $data): array
    {
        ...
        if (array_key_exists("cash", $data) && array_key_exists("raw", $data["cash"])) {
            $balance_sheet["cash"] = $data["cash"]["raw"];
        }

        if (array_key_exists("shortTermInvestments", $data) && array_key_exists("raw", $data["shortTermInvestments"])) {
            $balance_sheet["short_term_investments"] = $data["shortTermInvestments"]["raw"];
        }

        if (array_key_exists("netReceivables", $data) && array_key_exists("raw", $data["netReceivables"])) {
            $balance_sheet["net_receivables"] = $data["netReceivables"]["raw"];
        }

        if (array_key_exists("inventory", $data) && array_key_exists("raw", $data["inventory"])) {
            $balance_sheet["inventory"] = $data["inventory"]["raw"];
        }

        if (array_key_exists("otherCurrentAssets", $data) && array_key_exists("raw", $data["otherCurrentAssets"])) {
            $balance_sheet["other_current_assets"] = $data["otherCurrentAssets"]["raw"];
        }

        if (array_key_exists("totalCurrentAssets", $data) && array_key_exists("raw", $data["totalCurrentAssets"])) {
            $balance_sheet["total_current_assets"] = $data["totalCurrentAssets"]["raw"];
        }

        if (array_key_exists("longTermInvestments", $data) && array_key_exists("raw", $data["longTermInvestments"])) {
            $balance_sheet["long_term_investments"] = $data["longTermInvestments"]["raw"];
        }

        if (array_key_exists("propertyPlantEquipment", $data) && array_key_exists("raw", $data["propertyPlantEquipment"])) {
            $balance_sheet["property_plant_equipment"] = $data["propertyPlantEquipment"]["raw"];
        }

        if (array_key_exists("otherAssets", $data) && array_key_exists("raw", $data["otherAssets"])) {
            $balance_sheet["other_assets"] = $data["otherAssets"]["raw"];
        }

        if (array_key_exists("totalAssets", $data) && array_key_exists("raw", $data["totalAssets"])) {
            $balance_sheet["total_assets"] = $data["totalAssets"]["raw"];
        }

        if (array_key_exists("accountsPayable", $data) && array_key_exists("raw", $data["accountsPayable"])) {
            $balance_sheet["accounts_payable"] = $data["accountsPayable"]["raw"];
        }

        if (array_key_exists("shortLongTermDebt", $data) && array_key_exists("raw", $data["shortLongTermDebt"])) {
            $balance_sheet["short_long_term_debt"] = $data["shortLongTermDebt"]["raw"];
        }

        if (array_key_exists("otherCurrentLiab", $data) && array_key_exists("raw", $data["otherCurrentLiab"])) {
            $balance_sheet["other_current_liab"] = $data["otherCurrentLiab"]["raw"];
        }

        if (array_key_exists("longTermDebt", $data) && array_key_exists("raw", $data["longTermDebt"])) {
            $balance_sheet["long_term_debt"] = $data["longTermDebt"]["raw"];
        }

        if (array_key_exists("otherLiab", $data) && array_key_exists("raw", $data["otherLiab"])) {
            $balance_sheet["other_liab"] = $data["otherLiab"]["raw"];
        }

        if (array_key_exists("totalCurrentLiabilities", $data) && array_key_exists("raw", $data["totalCurrentLiabilities"])) {
            $balance_sheet["total_current_liabilities"] = $data["totalCurrentLiabilities"]["raw"];
        }

        if (array_key_exists("totalLiab", $data) && array_key_exists("raw", $data["totalLiab"])) {
            $balance_sheet["total_liab"] = $data["totalLiab"]["raw"];
        }

        if (array_key_exists("commonStock", $data) && array_key_exists("raw", $data["commonStock"])) {
            $balance_sheet["common_stock"] = $data["commonStock"]["raw"];
        }

        if (array_key_exists("retainedEarnings", $data) && array_key_exists("raw", $data["retainedEarnings"])) {
            $balance_sheet["retained_earnings"] = $data["retainedEarnings"]["raw"];
        }

        if (array_key_exists("treasuryStock", $data) && array_key_exists("raw", $data["treasuryStock"])) {
            $balance_sheet["treasury_stock"] = $data["treasuryStock"]["raw"];
        }

        if (array_key_exists("otherStockholderEquity", $data) && array_key_exists("raw", $data["otherStockholderEquity"])) {
            $balance_sheet["other_stockholder_equity"] = $data["otherStockholderEquity"]["raw"];
        }

        if (array_key_exists("totalStockholderEquity", $data) && array_key_exists("raw", $data["totalStockholderEquity"])) {
            $balance_sheet["total_stockholder_equity"] = $data["totalStockholderEquity"]["raw"];
        }

        if (array_key_exists("netTangibleAssets", $data) && array_key_exists("raw", $data["netTangibleAssets"])) {
    ...
    public function getRecommendations(string $symbol): array
    {
        ...
        if (($data["quoteSummary"]["error"] === null) && array_key_exists("recommendationTrend", $data["quoteSummary"]["result"][0]) && array_key_exists("trend", $data["quoteSummary"]["result"][0]["recommendationTrend"]) && array_key_exists(0, $data["quoteSummary"]["result"][0]["recommendationTrend"]["trend"])) {
            return $this->validateRecommendations($data["quoteSummary"]["result"][0]["recommendationTrend"]["trend"][0]);
        }
    ...
    public function getUpgradeDowngrade(string $symbol): array
    {
        ...
        if (($data["quoteSummary"]["error"] === null) && array_key_exists("upgradeDowngradeHistory", $data["quoteSummary"]["result"][0]) && array_key_exists("history", $data["quoteSummary"]["result"][0]["upgradeDowngradeHistory"])) {
            return $this->validateUpgradeDowngrade($data["quoteSummary"]["result"][0]["upgradeDowngradeHistory"]["history"]);
        }
